"""Criterion 4's `admission_intelligence` block -- the Insights-informed
implementation, moved here UNCHANGED on 2026-08-02 so the verbatim port of the
rest of Criterion 4 could be built around it without touching it.

WHY IT IS ITS OWN MODULE NOW
`criterion_4.py` was, until today, this code and nothing else. It is now the
full verbatim port of `server-scripts/UCC Analytics - Criterion 4.py` -- 40
questions, 81 metrics, the requirement registry and the 33 engine functions --
generated mechanically from the legacy source.

That port carries the legacy engine's OWN `resolve_source`, `resolve_field`,
`get_meta`, `clean_text` and friends, which are not the same functions as the
ones below. Leaving this code in the same module would have silently rebound
every call it makes to the legacy versions. So it moved out whole, keeping its
own helpers, and the ported engine calls into it at exactly the point the
legacy script called `build_admission_intelligence()`.

Nothing below this docstring was edited in the move. The text that follows is
the original module docstring, kept because it records what is verified
against live data and what is only faithfully translated -- which is still
true of this code and is still the thing worth knowing about it.
"""

"""Criterion 3 (Selection, Appointment, Management and Evaluation of Recruitment
Agents) analytics engine.

Ported from `server-scripts/UCC Analytics - Criterion 3.py` -- same technique as
`criterion_1.py`: the static config/registry block (legacy lines 81-1101) is
copied verbatim at module scope, and the engine/response-assembly code (legacy
lines 1122-1128, 1140-1984, 2191-2282) is wrapped one indentation level deeper
inside `run()`, with only the named mechanical transformations applied (global
scope -> function parameters, spaces -> tabs). Verified by
`tools/test_ucc_intelligence_criterion_3.py`, which re-extracts the same line
ranges from the live legacy source and diffs them against this file.

Criterion 3's engine has a genuinely different shape from Criterion 1's, not
just a different CONFIG -- confirmed by reading both scripts in full, not
assumed from Criterion 1's structure:

- Metrics are evaluated in two passes: base metrics first
  (`evaluate_base_metric`), then derived metrics (`evaluate_derived_metric`,
  modes `derived_sum`/`derived_percent`) that reference already-evaluated base
  metrics by id via `find_metric`/`metrics_by_id`. Criterion 1 has no
  equivalent derived-metric layer.
- Exception metrics are flagged inline per-metric (`"is_exception": True`)
  rather than collected in a separate `EXCEPTION_METRIC_IDS` list.
- There is no `REQUIREMENT_REGISTRY` / `evaluate_requirement` -- questions are
  answered directly from a linked metric via `answer_for_metric`, not from a
  separate requirement-evidence layer.
- `unsupported_metric(...)` is a static-catalogue helper that produces a
  placeholder metric entry for controls the current data model cannot yet
  evaluate (used directly inside `CONFIG`'s metric lists), not a runtime
  evaluation mode.
...
1. `clean_text`/`lower_text`/`is_truthy` are imported from `analytics.engine`
   instead of redefined locally -- verified byte-identical to Criterion 1's
   copies (content-diffed; the legacy files differ only in blank-line style
   between functions, which the extraction technique already tolerates).
2. `is_permission_error` is imported from `analytics.contracts` instead of
   redefined locally. Criterion 3's own legacy copy, like Criterion 1's, does
   not check for "403" -- the shared version's broader check is a Phase 2
   decision applied here, not re-decided.
3. `standardise_response_contract` is imported from `analytics.contracts`
   instead of redefined locally -- content-diffed against Criterion 3's own
   copy: only cosmetic differences (a reworded docstring, `CONTRACT_VERSION`/
   `ARRAY_KEYS` module constants instead of inline literals, trailing
   commas), confirming the Phase 2 hash-verification claim rather than just
   trusting it.
4. `row_cache = {}` (legacy line ~1285, inside the extracted engine range)
   becomes a local variable of `run()`, not a module-level global. This
   matters more than the equivalent closure decisions in Criterion 1: a
   Server Script's global namespace is freshly executed on every call, so
   `row_cache` legacy-starts empty every request; if this were ported as a
   module-level dict in a long-lived worker process instead, it would leak
   stale rows across separate requests (and, in a multi-user Frappe worker,
   potentially across users). Nesting the whole engine inside `run()` gives
   every call its own fresh `row_cache`, preserving the legacy behaviour
   rather than introducing a cross-request cache bug.
5. The legacy script ends by assigning its result to the Server Script
   ...
   (`api.py`'s `get_criterion_3`) is a `@frappe.whitelist()` method, and
...
Everything else -- every metric definition, every question, every condition,
every source/field candidate list, the two-pass base/derived evaluation
order, the drilldown dispatch -- is unchanged.
"""
...
def run(action, subcriterion, filters, metric_id, page, page_size, row_limit):
	"""Wrapped Criterion 3 engine -- see module docstring for the port technique
	and the five deliberate differences from the legacy Server Script."""
	def to_number(value):
		try:
			if value in [None, ""]:
				return None

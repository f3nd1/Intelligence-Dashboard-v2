"""Criterion 6 (Quality Assurance, Internal Audit, Management Review,
Innovation, Provider Evaluation, and Risk/Business Continuity) analytics
engine.

Ported from `server-scripts/UCC Analytics - Criterion 6.py` -- same
technique as the other three ported criteria. Verified by
`tools/test_ucc_intelligence_criterion_6.py`, which re-extracts the same
line ranges from the live legacy source and diffs them against this file.

**Same static-block exception as Criteria 2 and 7, confirmed by checking**:
the static config/registry block (legacy lines 61-1128 -- POLICY_REGISTRY
through STANDARD_FIELDS) is pprint-style, 252 of its 1058 non-blank lines
indented to something other than a multiple of 4 spaces. Copied
byte-verbatim. The per-line-graceful conversion introduced for Criterion 2
(convert only lines whose indent is a clean 4-space multiple, leave
anything else untouched) is used for the `run()` body here too, as the new
default going forward -- checked explicitly this time and found the
`run()`-body range has no irregular pockets of its own (unlike Criterion
2's "warnings" list), so the graceful function's fallback path never
triggers for this criterion, but it costs nothing to keep it as the
standard technique rather than reverting to the stricter one.

Criterion 6's engine, read in full before porting (2210 lines) -- the most
structurally distinct of the four criteria ported so far:

- **Parent/child metric split** (`evaluate_parent_metric` vs
  `evaluate_child_metric`, dispatched by `evaluate_metric` based on mode):
  `evaluate_child_metric` is new to this criterion -- for modes
  `child_count`/`child_parent_count`/`child_any_missing`, it calls
  `frappe.get_doc(parent_doctype, parent_name)` per matched parent row (not
  just `frappe.get_list`) to walk that parent's child table
  (`doc.get(table_field)`), evaluating conditions per child row. No other
  ported criterion touches child tables or calls `frappe.get_doc` at all.
- **`requirement_registry` aggregates across every subcriterion**, not just
  the requested one (legacy lines 2177-2193 loop `for criterion_key in
  QUESTION_REGISTRY`) -- every other ported criterion's `requirement_registry`
  action stays scoped to the current `subcriterion`. A genuine behavioural
  difference between criteria in the legacy system, not something this port
  introduces or should "fix" to match the others.
- `to_number` strips `","`/`"SGD"`/`"$"` before parsing (like Criterion 7's,
  unlike Criteria 1-3's plain `float(value)`).
- `filter_diagnostics = {}` (legacy line ~1371) is the same
  module-global-but-request-fresh shape flagged in Criteria 3 and 7 --
  becomes a local of `run()` via the same closure-nesting technique.
- Exceptions via `EXCEPTION_METRIC_IDS` (like Criteria 1, 2, 7). No
  requirement registry in the Criterion-1 sense (Criterion 6 has
  `requirements` built from `QUESTION_REGISTRY`, like Criteria 3/7's
  approach, not a separate authored registry).
...
1. `clean_text`/`lower_text`/`is_truthy` are imported from `analytics.engine`
   instead of redefined locally -- content-diffed against Criterion 6's own
   copies: identical apart from blank-line style.
2. `is_permission_error` is imported from `analytics.contracts` instead of
   redefined locally. Criterion 6's own legacy copy, like Criteria 1, 2 and
...
4. Every module-scope loop and intermediate variable (`resolved_sources`,
   `filter_diagnostics`, `metrics`, `metric_by_id`, `supporting_metrics`,
   `questions`, `sources`, `source_mapping`, `requirements`,
   `resolved_filters`/`unresolved_filters`, `data_quality`, `exceptions`,
   `readiness`, `available_sources`/`available_metrics`/
   `available_questions`, `criterion_overview`) becomes local to `run()`,
   not a module global.
5. The legacy script ends by assigning its result to the Server Script
   ...
   (`api.py`'s `get_criterion_6`) is a `@frappe.whitelist()` method, and
...
def run(action, subcriterion, filters, metric_id, page, page_size, row_limit):
	"""Wrapped Criterion 6 engine -- see module docstring for the port technique,
	the static-block indentation exception, the parent/child metric split, and
	the five deliberate differences from the legacy Server Script."""
	def to_number(value):
		try:
			if value in [None, ""]:
				return None

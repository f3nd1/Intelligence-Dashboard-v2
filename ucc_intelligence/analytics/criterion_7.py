"""Criterion 7 (Quality Management, Continual Improvement and Outcome
Measurement) analytics engine.

Ported from `server-scripts/UCC Analytics - Criterion 7.py` -- same technique
as `criterion_1.py`/`criterion_3.py`: the static config/registry block is
copied at module scope, and the engine/response-assembly code is wrapped one
indentation level deeper inside `run()`. Verified by
`tools/test_ucc_intelligence_criterion_7.py`, which re-extracts the same line
ranges from the live legacy source and diffs them against this file.

**One transformation is different here and is a deliberate, evidence-based
exception to the C1/C3 technique, not a shortcut**: the static block (legacy
lines 63-1188 -- POLICY_REGISTRY through STANDARD_FIELDS) is copied
byte-verbatim, without the usual 4-space -> tab conversion. Checked before
assuming it: 245 of the block's 1118 non-blank lines have indentation that is
not a multiple of 4 spaces. This isn't the hand-authored, evenly-indented
JSON-like style Criteria 1 and 3 use -- it reads as `pprint`/formatter output,
with continuation lines aligned to bracket columns rather than indent levels.
Forcing a tab conversion on that would either throw (most lines) or silently
misalign the block, so it stays as original spaces; only the `run()` body
(legacy lines 1209-1215, 1223-1798, 2005-2057, all confirmed regular 4-space
via the same check) gets the normal tab conversion. Mixing tabs and spaces
between two module-level regions of one file is syntactically fine in
Python 3 as long as no single block is internally ambiguous, which this
isn't.

Criterion 7's engine, read in full before porting (2061 lines): single-pass
metric evaluation (`evaluate_metric`/`base_metric`), no derived-metric layer
like Criterion 3's; exceptions collected via a separate `EXCEPTION_METRIC_IDS`
list like Criterion 1's, not inline flags like Criterion 3's; a
`required_value_coverage` metric mode not present in either Criterion 1 or 3;
a `display_doctype()` helper (`SOURCE_DISPLAY_NAMES` maps a backend DocType to
a friendlier label, e.g. "Quality Meeting" -> "Meeting Minutes") that neither
earlier criterion has.

**Same `row_cache`-shaped correctness note as `criterion_3.py`, flagged again
here because it recurred**: `filter_diagnostics = {}` and
`fetch_diagnostics = {}` (legacy lines ~1412-1413, inside the extracted engine
range) are module-level dicts in the Server Script, populated per-request
inside `fetch_rows`/`source_filter_details`. Harmless there because a Server
Script's global scope is freshly executed every call; ported as bare
module-level globals in a long-lived worker process, they would leak stale
filter/fetch state across requests and users. Nesting the whole engine inside
`run()` (the same technique used throughout this phase) gives every call its
own fresh copies, so this is preserved correctly by construction.
...
1. `clean_text`/`lower_text`/`is_truthy` are imported from `analytics.engine`
   instead of redefined locally -- content-diffed against Criterion 7's own
   copies: identical apart from blank-line style.
2. `is_permission_error` is imported from `analytics.contracts` instead of
   redefined locally. Unlike Criterion 1 and 3, this is an **exact** match,
   not a broadened one: Criterion 7's own legacy copy already checks for
   "403" (`analytics/contracts.py`'s shared version was generalised from
   Criterion 7's copy in Phase 2), so reusing it here changes nothing.
3. `standardise_response_contract` is imported from `analytics.contracts`
   instead of redefined locally -- content-diffed against Criterion 7's own
   copy: only cosmetic differences (reworded docstring, `CONTRACT_VERSION`/
   `ARRAY_KEYS`/`MIRROR_KEYS` module constants instead of inline literals,
   trailing commas).
4. `filter_diagnostics`/`fetch_diagnostics` (and `row_cache`-equivalent state)
   become locals of `run()`, not module-level globals -- see the note above.
5. The legacy script ends by assigning its result to the Server Script
   ...
   (`api.py`'s `get_criterion_7`) is a `@frappe.whitelist()` method, and
...
def run(action, subcriterion, filters, metric_id, page, page_size, row_limit):
	"""Wrapped Criterion 7 engine -- see module docstring for the port technique,
	the static-block indentation exception, and the five deliberate differences
	from the legacy Server Script."""
	def to_number(value):
		try:
			if value in [None, ""]:
				return None

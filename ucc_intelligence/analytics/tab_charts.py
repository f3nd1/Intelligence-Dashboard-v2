...
# --- the Records strip: drill-down, now that there are no cards -------------
#
# Under the per-card system the "show records" button lived on the card. A tab
# now shows one Insights dashboard, so the unit is the dashboard's own charts.
#
# THE CHAIN, AND ITS FOUR PERMISSION HOPS
#   1. the dashboard      get_doc + check_permission("read") -- the same check
#                         the iframe itself passes
#   2. linked_charts      child rows of a document just proven readable.
#                         Insights maintains this table from `items`; see
#                         insights_dashboard_v3.py, which declares
#                         `linked_charts: DF.TableMultiSelect[...]`
#   3. each chart         frappe.get_list, so a chart on the dashboard that
#                         this user cannot read is simply not listed
#   4. the records        drilldown.py, untouched -- frappe.get_list, never
#                         Insights' permission-blind execute()
#
# So the strip can only offer charts someone could already open in Insights,
# and can only return records they could already read in Desk.
#
# A chart that CANNOT be drilled is returned WITH ITS REASON rather than
# dropped. Someone looking at a chart with no button beside it should be told
# why, not left deciding whether it is a bug.

DASHBOARD_CHART_FIELD = "linked_charts"


def dashboard_drilldowns(dashboard):
	"""Every chart on one dashboard, and whether its records can be opened."""
	from ucc_intelligence.analytics import drilldown

	dashboard = clean_text(dashboard)
	if not dashboard:
		return {"ok": False, "charts": [], "message": "No dashboard given."}
	try:
		doc = frappe.get_doc(DASHBOARD_DOCTYPE, dashboard)
		doc.check_permission("read")
	except frappe.PermissionError:
		return {"ok": False, "charts": [], "message":
			"This dashboard is not available to your account."}
	except Exception:
		return {"ok": False, "charts": [], "message":
			"This dashboard is no longer in Insights."}

	rows = doc.get(DASHBOARD_CHART_FIELD) or []
	names = [clean_text(row.get("chart") if isinstance(row, dict) else getattr(row, "chart", ""))
		for row in rows]
	names = [name for name in names if name]
	if not names:
		# Not an error, and not silence either. Either the dashboard genuinely
		# holds no charts, or this Insights version does not maintain
		# linked_charts -- and those need different answers, so the message
		# says which field was empty rather than implying the first.
		return {"ok": True, "charts": [], "source": DASHBOARD_CHART_FIELD, "message":
			"Insights lists no charts on this dashboard, so there is nothing to "
			"open records for."}

	try:
		charts = frappe.get_list(chart_presentation.CHART_DOCTYPE,
			filters={"name": ["in", names]},
			fields=["name", "title", "query", "data_query"],
			limit_page_length=len(names)) or []
	except Exception:
		charts = []

	out = []
	for chart in charts:
		query = chart.get("query") or chart.get("data_query")
		resolved = drilldown.resolve(query) if query else {
			"status": "unsupported", "message": "This chart has no query behind it."}
		out.append({
			"chart": query or "",
			"title": clean_text(chart.get("title")) or chart["name"],
			"status": resolved.get("status"),
			"doctype": resolved.get("doctype") or "",
			"columns": resolved.get("columns") or [],
			"message": resolved.get("message") or "",
		})
	out.sort(key=lambda entry: (entry["status"] != "available", entry["title"].lower()))
	return {"ok": True, "charts": out, "source": DASHBOARD_CHART_FIELD}

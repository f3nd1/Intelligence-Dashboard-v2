"""Criterion 2 (Organisational Effectiveness -- Staff, Communication,
Information/Knowledge Management, and Feedback) analytics engine.

Ported from `server-scripts/UCC Analytics - Criterion 2.py` -- same technique
as `criterion_1.py`/`criterion_3.py`/`criterion_7.py`. Verified by
`tools/test_ucc_intelligence_criterion_2.py`, which re-extracts the same line
ranges from the live legacy source and diffs them against this file.

**Same static-block exception as `criterion_7.py`, confirmed again by
checking rather than assuming**: the static config/registry block (legacy
lines 63-974 -- POLICY_REGISTRY through STANDARD_FIELDS) is pprint-style,
with 498 of its 905 non-blank lines indented to something other than a
multiple of 4 spaces. Copied byte-verbatim, no tab conversion, for the same
reason as Criterion 7's.

**A genuinely new wrinkle found while porting this one**: the irregular
indentation isn't confined to the static block. The result dict's
`"warnings"` list (legacy lines 1795-1798, inside the otherwise-regular
`run()`-body range) is *also* pprint-style -- 3 of its 4 lines are
non-4-space-indented. This is harmless in the original Server Script and in
this port for the same reason the static block's irregularity is harmless:
indentation inside a bracket continuation (list/dict literal spanning
multiple lines) is not syntactically significant in Python, only cosmetic.
The port's line-conversion step handles this per-line rather than per-block:
any line whose indentation is a clean multiple of 4 spaces is tab-converted
as usual; any line that isn't (this `"warnings"` pocket) is left exactly as
in the legacy source. This is more robust than Criterion 7's all-or-nothing
per-block decision -- it would have caught this same wrinkle there too, had
it occurred, without needing a human to spot it by eye first.

Criterion 2's engine, read in full before porting (1827 lines): single-pass
metric evaluation (`evaluate_metric`), closest in shape to Criterion 1's
among the three criteria ported so far -- `resolve_field_groups`,
`metric_required_fields`, `row_matches` all present, same general structure.
Exceptions via a separate `EXCEPTION_METRIC_IDS` list (like Criterion 1 and
7, not Criterion 3's inline flags). No requirement registry (like Criterion
3 and 7). Eight subcriteria configured (2.1.1, 2.1.2, 2.2.1, 2.3.1, 2.3.2,
2.4.1, 2.4.2, 2.4.3) -- more than any criterion ported so far. Checked for
the same row_cache-style module-global risk flagged in Criteria 3 and 7:
none found here -- `fetch_rows` has no request-scoped cache dict, so there
is nothing extra to preserve via the `run()`-nesting technique beyond what
the technique already does for every module-scope loop.

Five deliberate differences from the legacy script, all already decided and
documented, not new decisions made here:

1. `clean_text`/`lower_text`/`is_truthy` are imported from `analytics.engine`
   instead of redefined locally -- content-diffed against Criterion 2's own
   copies: identical apart from blank-line style.
2. `is_permission_error` is imported from `analytics.contracts` instead of
   redefined locally. Criterion 2's own legacy copy, like Criterion 1 and
   3's, does not check for "403" -- the shared version's broader check is
   the Phase 2 decision applied here, not re-decided.
3. `standardise_response_contract` is imported from `analytics.contracts`
   instead of redefined locally -- content-diffed against Criterion 2's own
   copy: only the same cosmetic differences found in every other criterion
   so far (reworded docstring, extracted constants, trailing commas).
4. Every module-scope loop and intermediate variable (`resolved_sources`,
   `metrics`, `questions`, `sources`, `data_quality`, `exceptions`,
   `available_sources`, `available_metrics`) becomes local to `run()`, not a
   module global -- the same closure-nesting technique used throughout this
   phase, applied here even though (unlike Criteria 3 and 7) there was no
   specific cache-dict bug to avoid; it's still the correct general handling
   for a function that will be called repeatedly in a warm worker.
5. The legacy script ends by assigning its result to the Server Script
   response object. `run()` instead `return`s the result -- the API layer
   (`api.py`'s `get_criterion_2`) is a `@frappe.whitelist()` method, and
   Frappe populates the response message from a whitelisted method's return
   value automatically, so this is not a behaviour change.

Everything else -- every metric definition, every question, every source and
field candidate list, the evaluation logic -- is unchanged.
"""
...
def run(action, subcriterion, filters, metric_id, page, page_size, row_limit):
	"""Wrapped Criterion 2 engine -- see module docstring for the port technique,
	the static-block and warnings-list indentation exceptions, and the five
	deliberate differences from the legacy Server Script."""
	def to_number(value):
		try:
			if value in [None, ""]:
				return None

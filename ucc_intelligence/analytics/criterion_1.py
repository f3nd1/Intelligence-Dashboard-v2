"""Criterion 1 (Governance and Strategy) analytics engine.

Ported from `server-scripts/UCC Analytics - Criterion 1.py` -- the static
config/registry block (lines 63-784) is copied verbatim (module level here,
same as the legacy script's module scope), and the engine/business-logic
block (lines 801-1566, 1773-1842) is wrapped in `run()`, one indentation
level deeper, with the same mechanical transformation Phase 3 used for the
frontend: nothing in the body itself changed, only its containing scope.
Verified by `tools/test_ucc_intelligence_criterion_1.py`, which re-extracts
the exact same line ranges from the live legacy source and diffs them
against this file, so drift in either file is caught rather than assumed
away.

Four deliberate differences from the legacy script, all already decided
and documented, not new decisions made here:

1. `clean_text`/`lower_text`/`is_truthy` are imported from `analytics.engine`
   instead of being redefined locally -- verified byte-identical (or, for
   `clean_text`, identical to Criterion 1's own copy) across the seven
   legacy scripts before being pulled out (see `analytics/engine.py`'s
   docstring).
2. `is_permission_error` is imported from `analytics.contracts` instead of
   being redefined locally. That shared version also checks for `"403"` in
   the error text, which Criterion 1's own legacy copy did not -- a Phase 2
   decision (see `analytics/contracts.py`'s docstring), applied here rather
   than re-decided.
3. `standardise_response_contract` is imported from `analytics.contracts`
   instead of being redefined locally -- hash-verified byte-identical
   across all seven legacy scripts in Phase 2.
4. The legacy script ends by assigning its result to the Server Script
   response object (a Server Script-specific assumption). `run()` instead
   `return`s the result -- the API layer (`api.py`'s `get_criterion_1`) is a
   `@frappe.whitelist()` method, and Frappe populates the response message
   from a whitelisted method's return value automatically, so this is not a
   behaviour change, just removing a Server Script-specific assumption per
   CLAUDE.md's Phase 4 required work.

Everything else -- every metric definition, every question, every
condition, every field candidate list, the entire evaluation engine's
...
def run(action, subcriterion, filters, metric_id, page, page_size, row_limit):
	"""Compute the Criterion 1 analytics response for one request.

	Parameters match the legacy Server Script's module-level request
	variables exactly (`analytics/request.py` parses the raw payload into
	these) -- ported as parameters instead of module globals per CLAUDE.md's
	Phase 4 requirement to remove Server Script assumptions where a clearer

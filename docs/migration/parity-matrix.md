...
| X2 | `UCC Dashboard Access` configuration DocType | **verified 2026-07-26 not to exist** on `ucc.local` (`DoesNotExistError`) — the "manually created on site" premise this row carried was never actually checked until now | `[REBUILD]` | **Built** `ucc_intelligence/ucc_intelligence/sophia/doctype/ucc_dashboard_access/` (Phase 2, module folder + autoname/permissions confirmed by Felix, 2026-07-26) | `bench migrate` on `ucc-sms.orb.local` must create the DocType — not yet run |

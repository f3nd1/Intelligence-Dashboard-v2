...
| S8 | Shared response contract `2.1.0` | `standardise_response_contract` ×7 (byte-identical) | `[ADAPT]` | `ucc_intelligence/api/analytics/contract.py` (one copy) | Ported responses carry identical keys; `contract_version` unchanged |
| S9 | Ordered candidate source resolution + fallback | `resolve_source` ×7 | `[ADAPT]` | `ucc_intelligence/api/analytics/sources.py` | Same alias→DocType winner and same `resolution_attempts` for a given site |
| S10 | Permission-error classification | `is_permission_error` ×7 (C7 also matches `"403"`) | `[ADAPT]` | shared helper | Same status for the same Frappe error strings; decide C7's `"403"` explicitly |
...
| X8 | Shared permission-denied notice | `JAVASCRIPT.js:1-265` (`UCCShared`) | `[REUSE]` | `ucc_intelligence/public/js/shared.js` | `tools/test_permission_notice.js` (28 assertions) passes unchanged |
| X9 | Suppression of Frappe's raw permission dialog | `installPermissionMessageFilter` | `[REUSE]` | same | Raw `PermissionError` text never reaches the visible body |
...
| P5 | Zep adapter interface stub | — (new) | — | `ucc_intelligence/memory/zep_adapter.py` | Zero network calls; no `zep`/`requests`/`httpx` import |

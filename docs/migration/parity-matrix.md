...
| X2 | `UCC Dashboard Access` configuration DocType | **verified 2026-07-26 not to exist** on `ucc.local` (`DoesNotExistError`) — the "manually created on site" premise this row carried was never actually checked until now | `[REBUILD]` | **Revised (O7): create fresh, not convert.** Drafted at `docs/migration/phase-2-doctype-draft/`, not yet placed — module-name folder depth still needed; `autoname`/`permissions` are now a design decision (no existing DocType to copy them from), see `phase-2-plan.md` §5 | DocType installs with the app; on a site where it never existed, there are no rows to preserve |
...
| O7 | **Revised (2026-07-26).** Felix's original decision — reuse the existing DocType, convert to app-managed — assumed one exists on the live site. Verified via `bench console` on `ucc.local` that it does not (`DoesNotExistError`). Revised question: create a fresh app-managed DocType matching the Server Script's documented field shape (still the right call — nothing changes about *that*), but `autoname`/`permissions` are now a genuine design decision rather than something to copy from a live schema, and need Felix's sign-off rather than a live-site dump. Module-name folder depth is still an open fact, unrelated to this finding. See `phase-2-plan.md` §5. | X1, X2 |

...
| X2 | `UCC Dashboard Access` configuration DocType | **verified 2026-07-26 not to exist** on `ucc.local` (`DoesNotExistError`) — the "manually created on site" premise this row carried was never actually checked until now | `[REBUILD]` | **Built and confirmed** `ucc_intelligence/ucc_intelligence/sophia/doctype/ucc_dashboard_access/` (Phase 2) | `bench migrate` on `ucc-sms.orb.local` created the DocType; `frappe.get_doc("DocType", "UCC Dashboard Access")` resolves it — confirmed 2026-07-26 |
...
| O7 | **Resolved (2026-07-26).** DocType didn't exist on `ucc.local` (verified) — created fresh instead of converted. Module folder confirmed `sophia` (matching `modules.txt`'s `Sophia`); `autoname: hash` and `permissions: System Manager only` confirmed by Felix. Placed at `sophia/doctype/ucc_dashboard_access/`. See `phase-2-plan.md` §5. | X1, X2 |

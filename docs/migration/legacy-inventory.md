...
|---|---|---:|---|---|
| `UCC Dashboard Access.py` | `ucc_dashboard_access` | 259 | `[ADAPT]` | **Port early.** Closest legacy analogue to Phase 1's role-restricted workspace. Depends on a `UCC Dashboard Access` DocType that today exists only as a manually-created site object — it must become a real app DocType with a fixture. Uses `frappe.get_all(..., ignore_permissions=True)` on `Has Role`; re-justify that on port. Fail-open by design (`default_when_unconfigured`). |

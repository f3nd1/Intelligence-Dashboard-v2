...
| 3 | Migrate shared error classification + blocked-source presentation (JS) | `src/js/00-shared-runtime.js` (251 lines, read in full) | `ucc_intelligence/ucc_intelligence/public/js/shared.js` — byte-identical straight copy (diffed, confirmed identical). Smoke test at `ucc_intelligence/ucc_intelligence/tests/test_shared_runtime.js`. Path corrected 2026-07-26: originally placed one level too high (sibling of the inner package, matching CLAUDE.md §7's aspirational tree) — real `bench build` output confirmed the actual convention links `<app>/<app>/public` to assets, not `<app>/public`. See Phase 3 fix notes. |
